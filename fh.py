#!/usr/bin/python3
# -*- coding: utf-8 -*-
import re, yaml, argparse, asyncio, os, sys
from astarmiko.baseAI import (
    Activka,
    port_name_normalize,
    convert_mac,
    is_ip_correct,
...
def check_firewall(myactivka, ip, routerstart):
    ...
    Args:
        ...
        routerstart (str): the name of the device that is the router for the host network
    
    Returns:
        output (list):    list [mac address of host with defined ip, ip_address_of_firewall]
    ...
    if route:
        if myactivka.choose(routerstart, withoutname = True)['device_type'] == 'cisco_ios':
        ...
        #print(f'DEBUG: in check_firewall koid = {koid} len(koid) = {len(koid)}  {type(koid[0])}')
        if len(koid) == 1 and type(koid[0]) == str:
            intf_indx = koid[0].split(' = ')[1]
            #print(f'DEBUG: in check_firewall intf_indx = {intf_indx}')
            oid = OID_INFF_MAC_STR.format(intf_indx, ip)
            #print(f'DEBUG: in check_firewall oid = {oid}')
            mac = asyncio.run(snmp_get_oid(fw_ip, community, oid))
            mac = mac[0].split(' = ')[1]
            mac_str = ':'.join(f'{(int(mac, 16) >> 8*i) & 0xff:02x}' for i in reversed(range(6)))
...
def get_host_description_ad(hostname):
    '''
    Function get name of hostname and return it description from ADDS if it exist
    or string "not found" if not.
    '''
    ...
    #print(f'DEBUG fh get_host_description_ad comp = {comp}')
    if comp:
...
def findbehindfw(myactivka, m):
    ...
    Args:
        ...
        m[2] (condt):        'FIREWALL'
        m[3] routerstart (str): the name of the device that is the router for the host network
    ...
    if myactivka.levels[m[3]] != 'R':
        #if routertostart is L3 swith lookup in his mac addres table by router IP
        ...
        output = out[0] #output = [fw_ip, fw_mac, port_where_this_mac_is_lit']
    return output
...
def findchain(myactivka, m, hostname = False):
    ...
    host with that mac 
    '''
    ##print(f'DEBUG findchain got var m ={m} hostname = {hostname}')
    global message
    end = []
    i = 0
    mac_to_find = m[1]
    ...
    if hostname and os.name == 'nt':
        ...
        return_text = [message[20].format(hostname, correct_ip, mac_to_find, '\n', m[3], m[2], '\n', description)]
    else:
    ...
    if not myactivka.levels[m[3]] == 'CH':
        ...
        if myactivka.levels[m[3]] == 'R':
            ...
            ##print(f'DEBUG findchain found first sw ={sw}')
        else:
            sw = m[3]
    else:
        if m[3] == 'ChPSever-HA':
    ...
    while True:
        ##print(f'DEBUG findchain got var sw ={sw}')
        ...
        ##print(f'DEBUG fh port = {port} len(port) = {len(port)}')
        if not port[1]:
            next_neighbor = myactivka.getinfo(sw, 'neighbor_by_port', port[0])
            ##print(f'DEBUG findchain here: если за портом много устройств но ни по CDP ни по LLDP соседа не получаем, значит там “тупой” неуправляемый коммутатор, останавливаемся и сообщаем об этом')
...
def find_router_to_start(myactivka, ip, is_mac = False, router = None):
    ...
    if not is_mac:
        ...
        ##print(f'DEBUG fh_router_to_start nbi = {nbi}')
    ...
    ##print(f'DEBUG fh_router_to_start routerstart = {routerstart}  out = {out}')
    if is_mac:
    ...
    #print(f'DEBUG fh_router_to_start return output = {output}')
    return output
...
def ip_routine(myactivka,ip):
    ...
    ##print(f'DEBUG fh_ip_routine correct_ip = {correct_ip}')
    ...
    if not m:
        if correct_ip in myactivka.routerbyip.keys():
            ...
            name_mask = [[line[0], line[2]] for line in int_conf if line[1] == correct_ip ]
            print_string = message[25].format(correct_ip, name_mask[0][1], name_mask[0][0], device2)
            print(print_string)
            quit()
        print(message[13])
        quit()
    print(f'DEBUG ip_routine have m ={m} hostname = {hostname}')
    if m[2] == 'FIREWALL':
        #if firewall m = [ip, mac_of_this_ip, 'FIREWALL', routerstart, ip_add_of_firewall]
