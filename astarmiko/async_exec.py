...
class ActivkaAsync(Activka):
    async def execute_on_devices(
        ...
        commands: Union[str, List[str], Dict[str, List[str]]],
        rsyslog=False,
        loki=False,
        elastic=False,
    ) -> Dict[str, Any]:
        ...
        await tqdm_asyncio.gather(
            *(worker(dev) for dev in devices), desc="Executing show commands"
        )
        return results

    async def setconfig_on_devices(
        self,
        devices: Union[str, List[str]],
        commands: Union[str, List[str], Dict[str, List[str]]],
        rsyslog=False,
        ...
        await tqdm_asyncio.gather(
            *(worker(dev) for dev in devices), desc="Executing config commands"
        )
        return results
