...
def _try_connect(device, func, *args, **kwargs):
    ...
    def connect_with_credentials(device_params):
        ...
        try:
            ...
            logger.warning(f"Authentication failed for {device_params['username']}@{device_params['ip']}")
...
def send_commands(device, commands, mode='exec', validate_each=False):
    ...
    Args:
        device (dict): Netmiko-style device dict
        commands (str or list): commands to send
        mode (str): 'exec' or 'config'
        validate_each (bool): validate each config command separately

    Returns:
        str or tuple: result or (good, failed) for validate_each
    """
    ...
    def config_mode(ssh):
        if validate_each:
            good, failed = {}, {}
            errors = re.compile(r'Invalid input|Incomplete|Ambiguous|Unrecognized')
            for cmd in commands:
                res = ssh.send_config_set(cmd)
                if errors.search(res):
                    failed[cmd] = res
                else:
                    good[cmd] = res
            return good, failed
...
class Activka:
    ...
    def _is_device_available(self, device: dict) -> bool:
        ...
        try:
            ...
            logger.warning(f"Device availability check failed for {device['host']}: {str(e)}")
            return False
...
class ActivkaBackup(Activka):
    ...
    def _write_backup_scp(self, segment: str, filename: str, content: str, second: bool = False):
    ...
    def _write_backup_sftp(self, segment: str, filename: str, content: str, second: bool = False):
    ...
    def _get_backup_config_scp(self, segment: str, device: str, second: bool = False) -> List[str]:
